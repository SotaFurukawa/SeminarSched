...
このチェックリストは `v1.3.0` Releaseを検証するためのものです。
...
- [ ] Release tagはソース内versionと一致する `v1.3.0` である。
...
- [ ] artifact名が `SummerCourseScheduler-Portable-1.3.0.zip` と一致する。
...
- [ ] artifact名が `SummerCourseScheduler-Setup-1.3.0.exe` と一致する。
