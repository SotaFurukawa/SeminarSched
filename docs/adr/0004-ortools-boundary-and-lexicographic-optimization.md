...
時間割には、受講・出勤可能日時、科目資格、重複禁止、最大2名、1対1必須、
...
1. 未配置授業数の最小化
2. 通常担当講師の目標不足を優先度5、4、3、2の順に最小化
3. 通常担当、第1〜3希望講師の違反最小化
4. 同じ生徒・科目を担当する講師数の最小化
5. 必要回数8回未満の同日集中の最小化
6. 複数月にまたがる講習期間への分散最大化
7. 稼働する`講師 × 日付 × コマ`数の最小化
8. 生徒・講師の希望日時の最大化
9. 既存時間割からの変更最小化
10. 勤務可能枠に対する講師参加割合の任意調整
...
候補生成後、ロック済み授業を先に保持し、候補数、講師希望、1対1必須等を考慮した
...
段階内weight、時間上限、乱数seed、search worker数はYAML設定からDTOへ注入する。
同一講師が通常担当と希望順位の複数区分へ現れる場合は最大点だけを採用し、重複加点
しない。v1.7.0では通常担当へ固定の小加点2を付け、希望順位との均衡を少しだけ通常担当側へ
寄せる。
