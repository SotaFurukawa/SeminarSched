...
このチェックリストは `v1.4.1 (Beta)` Releaseを検証するためのものです。
...
- [ ] Release tagはソース内versionと一致する `v1.4.1` であり、Pre-releaseである。
...
- [ ] artifact名が `SummerCourseScheduler-Portable-1.4.1.zip` と一致する。
...
- [ ] artifact名が `SummerCourseScheduler-Setup-1.4.1.exe` と一致する。
