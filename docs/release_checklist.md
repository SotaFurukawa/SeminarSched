...
このチェックリストは `v1.8.1 (Beta)` Releaseを検証するためのものです。
...
- [ ] Release tagはソース内versionと一致する `v1.8.1` であり、Pre-releaseである。
...
- [ ] artifact名が `SummerCourseScheduler-Portable-1.8.1.zip` と一致する。
...
- [ ] artifact名が `SummerCourseScheduler-Setup-1.8.1.exe` と一致する。
