...
現在は **v1.0.1の公開準備版** です。GitHub Releaseを公開するまでは配布版を
意味しません。Phase 1の
...
- `SummerCourseScheduler-Setup-1.0.1.exe`
- `SummerCourseScheduler-Portable-1.0.1.zip`
- `SHA256SUMS.txt`

ダウンロード後は、同梱一覧と実ファイルのSHA-256を照合します。

```powershell
Get-FileHash .\SummerCourseScheduler-Setup-1.0.1.exe -Algorithm SHA256
Get-FileHash .\SummerCourseScheduler-Portable-1.0.1.zip -Algorithm SHA256
```
...
`dist\SummerCourseScheduler-Portable-1.0.1.zip`を作ります。QML、Qt plugin、
...
.\.venv-release\Scripts\python.exe scripts\package_release.py checksums `
  ...
  dist\SummerCourseScheduler-Portable-1.0.1.zip `
  dist\SummerCourseScheduler-Setup-1.0.1.exe
