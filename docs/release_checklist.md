...
このチェックリストは `v1.1.0` Releaseを検証するためのものです。
...
- [ ] Release tagはソース内versionと一致する `v1.1.0` である。
...
- [ ] artifact名が `SummerCourseScheduler-Portable-1.1.0.zip` と一致する。
...
- [ ] artifact名が `SummerCourseScheduler-Setup-1.1.0.exe` と一致する。
