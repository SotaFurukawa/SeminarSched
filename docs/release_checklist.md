...
このチェックリストは `v1.6.0 (Beta)` Releaseを検証するためのものです。
...
- [ ] Release tagはソース内versionと一致する `v1.6.0` であり、Pre-releaseである。
...
- [ ] artifact名が `SummerCourseScheduler-Portable-1.6.0.zip` と一致する。
...
- [ ] artifact名が `SummerCourseScheduler-Setup-1.6.0.exe` と一致する。
...
- [ ] 生徒・講師アンケート。
- [ ] 集団授業の操作画面・新規登録が表示されない（v2.0.0で再導入予定）。
- [ ] 入力検証。
- [ ] 自動最適化。
- [ ] 未配置・理由。
- [ ] 手動編集。
