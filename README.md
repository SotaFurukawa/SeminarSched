...
ロック以外の全体再最適化に加え、共通レイアウトからのExcel・PDF出力を
追加しています。Phase 7では自動バックアップ世代管理、破損検出と安全な復元、
版表示、Windowsポータブル版・インストーラー・SHA-256を作る配布基盤、性能・
運用・受入文書を追加しました。

Phase 4の最適化画面はPhase 5の編集画面から開けます。「出力」では全体時間割、
生徒配布時間割、講師配布時間割（学年順・講師別）、未配置・警告一覧をExcel / PDFで
生成できます。選択日・選択生徒・
選択講師だけの部分再最適化は、安全な
境界が確定するまで提供していません。
...
刷新UIでは、ホームとサイドバーに次の6段階を常時表示します。
...
姓を入力すると、最小の未使用ID・最大連続2・空きなしの既定値を表示します。空行の在籍は
...
にまとめています。通常は②「アンケート作成」で現在の講習設定を反映した
...
- 通常担当講師、担当優先度、1対1契約をアンケートから上書きしない保護
...
- 開校日、有効コマ、0／1／2のavailability、講師資格、
  固定授業を考慮した疎な候補生成
- 必要回数ごとの配置または未配置、生徒重複禁止、講師最大2名、1対1必須、
  生徒・講師の空きコマ禁止、
  生徒の連続上限等をCP-SATのハード制約として適用
- 未配置数を最優先にし、通常担当の優先度別目標率（0／25／50／75／100%）を
  優先度5から順に最大限満たしたうえで、担当講師数・同日集中・複数月への分散も評価する
  段階的な辞書式最適化
- 通常担当が欠席して目標率に届かない場合は、資格と出勤条件を満たす代講へ配置し、
  生徒・科目・目標回数・実績回数を「確認が必要な項目」と「未配置・警告」に表示
...
受講希望などの業務データは、利用者別のプロジェクトフォルダーでアプリが管理する
`.jukuschedule`ファイルへ保存されます。「名前を付けて保存」では任意の場所へ複製できます。
両者を同じDBとして扱わないでください。
...
ホームの「生徒・講師の基本情報（共通）」から操作します。設定画面にはExcel入出力を
置かず、講習を開いていないときも同じ場所からテンプレートを保存・取込みできます。

- ファイル名は既定で`生徒・講師_基本情報.xlsx`です。
- シートは「生徒」「講師」「科目」「講師対応科目」「通常授業」の5枚です。
- 1行目は日本語ヘッダーです。2行目以降には入力用のプルダウン、参照式、既定値が
  用意されます。
- 真偽値は原則「はい／いいえ」で入力します。IDは名前ではなく安定した文字列IDを
  使用します。
- 「講師対応科目」「通常授業」は名前・科目名をプルダウンで選ぶと、隣のID・科目コードが
  自動表示されます。登録済み行は安定IDから氏名・科目名を表示するため、生徒・講師・
  科目シートで名称を変更すると自動追従します。確認専用の重複列はありません。
- 通常授業の担当講師優先度は空欄で`3`、1対1必須は空欄で`いいえ`、講師対応科目の
  指導可能は空欄で`はい`として反映します。
...
- エラーが1件でもあるプレビューは反映できません。警告だけの場合は、内容を確認して
  から反映できます。
- 取込み前の共通正本は`基本情報バックアップ`へ
  `生徒・講師_基本情報(by日時).xlsx`として退避します。取込んだブックはworkspaceの
  `生徒・講師_基本情報.xlsx`へ正規化してコピーされ、「反映済みの基本情報を開いて編集」は
  常にこの正本を開きます。現在の講習へのDB反映は1トランザクションで行います。
...
科目コードと第1～第3希望講師を含みますが、通常担当、担当優先度、1対1契約は塾側
...
Googleフォーム作成手順は、モーダル内の左下にある「別ウィンドウで表示」から独立した
ウィンドウへ切り替えられます。取込み後は、③画面の「取込み済み回答を編集…」から
学年で絞り込んだ複数生徒を選択し、開校日の各コマを参加可／参加不可へ一括変更できます。
休校日は授業を配置しないため編集対象外です。
...
自動またはボタンで開けます。
全体時間割、生徒配布時間割、講師配布時間割（学年順・講師別）、未配置・警告一覧を
ExcelまたはPDFとして保存できます。講師別は講師ごとのファイルをフォルダーへまとめます。
生徒ごとの配布帳票は講習参加者だけにカレンダーを作り、不参加者は先頭の
「講習欠席一覧」へまとめます。講師配布用の帳票には、生徒名とカレンダーの間へ
科目略称と通常担当講師名を表示します。日付、講師、生徒は必要な対象だけを選択できます。
