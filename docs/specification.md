...
Phase 0からPhase 7までの段階的な開発項目を実装し、現在は`1.8.1 (Beta)`の
...
- 開校日、有効コマ、availability、資格、任意時刻の集団授業、
  ロック済みAssignmentを用いた疎な候補生成
- 各sessionの配置または未配置、生徒重複禁止、講師最大2名、1対1必須、固定、
  集団授業、生徒連続上限、生徒・講師の空きコマ禁止をCP-SATのハード制約として実装
- 未配置、通常担当の優先度別目標不足（優先度5→2）、講師希望、担当講師数、
  同日集中、複数月への分散、勤務可能枠に対する
  講師実稼働率の公平化、稼働講師枠、希望日時、既存割当変更の順による
  複数Solveの辞書式最適化
...
- 日曜始まり・土曜終わりの週ごとに1シート（PDFでは1ページ）へ分け、日付を2面ずつ
  横並びにする全体時間割。各日には1コマ以上勤務可能な講師だけを4名単位で表示し、
  勤務不可コマを灰色にする。日付、コマ／時刻、最大2名分の生徒名・学年・科目、
  1対1、集団、休校、ロック、手動変更、警告、未確定、凡例、校舎、講習名、
  更新日時、ページ番号を扱う
...
- app version `1.8.1`とrelease channel `Beta`をpackage metadata、Qt application、
  About、log、帳票へ
