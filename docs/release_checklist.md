...
このチェックリストは `v1.3.4` Releaseを検証するためのものです。
...
- [ ] Release tagはソース内versionと一致する `v1.3.4` である。
...
- [ ] artifact名が `SummerCourseScheduler-Portable-1.3.4.zip` と一致する。
...
- [ ] artifact名が `SummerCourseScheduler-Setup-1.3.4.exe` と一致する。
