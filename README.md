...
現在のアプリ版は **v1.6.3 (Beta)** です。v1系はすべてBeta版としてGitHubの
...
- `SummerCourseScheduler-Setup-1.6.3.exe`
- `SummerCourseScheduler-Portable-1.6.3.zip`
- `SHA256SUMS.txt`

ダウンロード後は、同梱一覧と実ファイルのSHA-256を照合します。

```powershell
Get-FileHash .\SummerCourseScheduler-Setup-1.6.3.exe -Algorithm SHA256
Get-FileHash .\SummerCourseScheduler-Portable-1.6.3.zip -Algorithm SHA256
```
...
`dist\SummerCourseScheduler-Portable-1.6.3.zip`を作ります。QML、Qt plugin、
...
.\.venv-release\Scripts\python.exe scripts\package_release.py checksums `
  ...
  dist\SummerCourseScheduler-Portable-1.6.3.zip `
  dist\SummerCourseScheduler-Setup-1.6.3.exe
