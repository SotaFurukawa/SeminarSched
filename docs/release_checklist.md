...
このチェックリストは `v1.6.7 (Beta)` Releaseを検証するためのものです。
...
- [ ] Release tagはソース内versionと一致する `v1.6.7` であり、Pre-releaseである。
...
- [ ] artifact名が `SummerCourseScheduler-Portable-1.6.7.zip` と一致する。
...
- [ ] artifact名が `SummerCourseScheduler-Setup-1.6.7.exe` と一致する。
